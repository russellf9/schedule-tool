        ...
        link: function(scope, element, attrs) {
            function doClick() {
                ...
                if (navigator.appName == 'Microsoft Internet Explorer') {
                    // add the iframe element to write the data too
                    element.after('<iframe id="myFrame" style="display:none"></iframe>');

                    var doc = document.querySelector('#myFrame').contentDocument;

                    doc.open('application/CSV', 'replace');
                    doc.charset = 'utf-8';
                    doc.write(scope.csv);
                    var success = doc.execCommand('SaveAs', true, scope.getFilename());
                    if (success){
                        doc.close();
                    }
                } else {
